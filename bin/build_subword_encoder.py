...
from meansum.data_loaders import text_encoder, tokenizer
from meansum.data_loaders.summ_dataset_factory import SummDatasetFactory
from meansum.project_settings import RESERVED_TOKENS, HParams
...
def main(opt):
    ...
    if opt.dataset:
        ...
        token_counts = tokenizer.corpus_token_counts(
            opt.corpus_filepattern, opt.corpus_max_lines, split_on_newlines=True
        )
    else:
    ...
    vocab_fp = os.path.join(
        opt.output_dir, opt.output_fn + ".txt"
    )  # stores vocab coutns
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--corpus_filepattern", default=None, help="Corpus of one or more text files"
    )
    parser.add_argument(
        "--corpus_max_lines",
        default=float("inf"),
        help="How many lines of coprus to read",
    )
